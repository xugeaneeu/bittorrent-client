...
import bencode.bencoding.Reader;
import bencode.bencoding.Utils;
import bencode.bencoding.types.*;
...
{
  public static TorrentMeta parseTorrent(InputStream input) throws IOException
  {
    Reader r = new Reader(input);
    return parseTorrent(r);
  }

  public static TorrentMeta parseTorrent(String filePath) throws IOException
  {
    Reader r = new Reader(new File(filePath));
    return parseTorrent(r);
  }

  public static TorrentMeta parseTorrent(Reader r) throws IOException
  {
    List<IBencodable> x = r.read();
    // A valid torrent file should only return a single dictionary.
    if (x.size() != 1)
  ...
  private static TorrentMeta parseTorrent(Object o) throws ParseException
  {
    if (o instanceof BDictionary torrentDictionary)
    {
      ...
      t.setFileLength(parseSingleFileTotalSize(infoDictionary));
